...
- 初始化
  1. `int arr[N]={n}`则第一个元素被初始化为n,其余元素初始化为0
   <br>

  2. 多维数组初始化,第一维可由初始化列表推断,其他维必须显式指定
     ```c++
     //以二维数组为例
...
- 指针"错误"类型
  1. **悬挂指针:**`delete`指针指向内存后未将指针置为`nullptr`
   <br>

  2. **野指针:** 指针定义时没有初始化,随机指向内存中某个位置
  <br>

- 几类指针特性
  1. **`void*`指针**不可以直接赋给(隐式转换)非`void`指针
     ```C++
...
- 指针与函数:函数不可返回**局部变量指针**,常使用`new`向堆区申请
<br>
...
- 类的多态
  1. **虚函数**:构造函数不能是虚函数,析构函数一般是虚函数
   <br>
  2. **基类**指针/引用可以指向**派生类**对象,但只能调用从**基类继承的成员**
   <br>
  3. 在派生类中写基类同名函数
...
- 赋值运算`=`实现连续赋值`m1=m2=m3`:
  1. 返回值需是**对象引用**:`m1=(m2=m3)`,`m2=m3`需返回`m2`的引用才能继续赋值
   <br>

  2. 参数是对象的引用:避免不必要的拷贝,通常使用`const M&`
   <br>
...
- 插入运算符`<<`
  1. 左端为输入流,且访问类私有属性,则一般声明为类友元函数
   <br>

  2. 返回值类型为`ostream&`支持连续链式输出
   <br>

  3. 函数参数为`ostream&`与类对象常引用,避免不必要拷贝
...
- static与类
  ...
  2. 静态方法不能直接访问非静态成员<br>静态方法无`this`指针,无法直接访问非静态成员
      ```C++
      class Example{
         static int val1;
         int val2=5;
...
- 常量只能由常量指针来指向,由常引用来引用
<br>

### 友元

- 友元类
  1. 在类型A中声明`friend class B`是A授权**允许B访问A的私有成员**
     ```C++
     class B; //前置声明
     class A{
        int valA=10;
...
- 友元函数
  1. 友元函数在类内声明但不是该类的成员函数<br>本质是授权**允许函数访问类的私有成员**
     ```C++
     class Example{
     private:
        int val = 10;
  ...
  2. 将重载函数声明为友元函数,只会针对特定重载版本,而非所有
<br>
