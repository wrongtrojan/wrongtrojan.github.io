...
把样本排成矩阵之后，协方差就是 $XX^{\top}$ 的常数倍。把方差最大化和重构误差最小化写开成 Rayleigh 商，其实是同一组特征向量的两种说法。
...
若 $S$ 的条件数很大，某些方向方差极小，主子空间估计容易被噪声带偏。这与上一篇的病态分析是同一件事：$\kappa_{2}(S)=\lambda_{\max}/\lambda_{\min}$，而 $\lambda_{i}$ 与数据矩阵奇异值的平方成比例。
...
$A_{k+1}$ 的主特征向量就是原来的 $u_{k+1}$。在 SVD 语言里，这不过是每次扔掉一个 $\sigma_i u_i v_i^{\top}$。实际计算不必真的做这种减法——一次特征分解或一次截断 SVD 就给出全部 $u_1,\ldots,u_r$——但收缩把“逐个取出主方向”的几何说清楚了：每一步都在剩余的正交补里重复同一个 Rayleigh 问题。
...
左边是常数，所以最大化第一项与最小化第二项是同一件事。两者都由同一组特征向量（左奇异向量）实现。这就是 PCA 能同时具有统计解释（方差）和几何解释（投影误差）的原因。

实现上，$d$ 小、$N$ 大时分解 $XX^{\top}$；$d$ 大或稀疏时直接截断 SVD。中心化是必做的；是否把各特征标准化成单位方差，取决于任务是否关心量纲——标准化之后 PCA 看的是相关结构而不是原始方差结构。

到这里，前面几篇的主线收拢：SVD 提供谱分解与伪逆，范数提供误差尺度，Eckart–Young–Mirsky 提供最优低秩逼近，条件数提供稳定性判断，协方差把统计目标映射回对称半正定矩阵。PCA 就是“谱主方向 + 误差控制”的降维方法。
...
范数定义误差尺度，SVD 给出谱结构，条件数刻画可解性与敏感性，QR / SVD 相对法方程实现稳定求解，协方差与 PCA 完成统计降维闭环。
